#!/usr/bin/env python3
"""
Constant Weight PINN Training Script

This script implements a baseline PINN training with constant weights for loss balancing.
This serves as a baseline comparison for the adaptive loss balancing methods.

Usage:
    python constant_weight_pinn_training.py [--epochs 2000] [--batch-size 256] [--lr 1e-4] [--max-samples 1000]
"""
...
class ConstantWeightLoss(nn.Module):
    """
    Constant Weight PINN loss balancing method.
    
    This method uses fixed equal weights for all loss components,
    providing a baseline comparison for adaptive methods.
    """
    
    def __init__(self, enable_mass_constraints: bool = True, weights: dict | None = None):
        super().__init__()
        ...
        default_weights = {key: 1.0 for key in self.loss_keys}
        self.current_weights = {**default_weights, **(weights or {})}
    ...
    def step(self, loss_components, optimizer, model, X_batch, y_batch):
        """
        Performs constant weight optimization step:
        1. Calculate weighted total loss (data + mean(physics))
        2. Optimize network parameters
        """
        # Extract data and physics losses
        data_loss = loss_components[0]
        physics_components = loss_components[1:]
        
        # Weighted data term
        w_data = float(self.current_weights.get('data', 1.0))
        weighted_data = w_data * data_loss
        
        # IMPORTANT: adaptive_custom_loss always returns 7 components (data + 4 physics + 2 mass)
        # We need to handle this correctly regardless of enable_mass_constraints
        num_physics_components = len(physics_components)
        
        # Create physics weights tensor with the correct size
        # For synthetic data (no mass constraints): use only first 4 physics components
        # For real data (with mass constraints): use all 6 physics components
        if num_physics_components == 6:  # Real data: 4 physics + 2 mass
            physics_keys = ['phys_res1', 'phys_res2', 'phys_res3', 'phys_res4', 'phys_mass1', 'phys_mass2']
        else:  # Synthetic data: only 4 physics (mass constraints are zeros)
            physics_keys = ['phys_res1', 'phys_res2', 'phys_res3', 'phys_res4']
        
        # Ensure we have weights for all physics components
        physics_weights = []
        for key in physics_keys:
            weight = float(self.current_weights.get(key, 1.0))
            physics_weights.append(weight)
        
        # Pad weights if needed (shouldn't happen, but safety check)
        while len(physics_weights) < num_physics_components:
            physics_weights.append(1.0)
        
        # Convert to tensor
        physics_weights = torch.tensor(physics_weights, 
                                     dtype=physics_components[0].dtype, 
                                     device=physics_components[0].device)
        
        # Stack physics components
        phys_losses = torch.stack(physics_components)
        
        # Compute weighted mean of physics losses (normalize by sum of weights)
        sum_w = torch.clamp(physics_weights.sum(), min=1e-12)
        weighted_phys_mean = (phys_losses * physics_weights).sum() / sum_w
        
        # Total loss combines weighted data and weighted physics mean
        total_loss = weighted_data + weighted_phys_mean
...
def add_method_arguments(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    # Per-component weights (defaults to 1.0)
    parser.add_argument('--w-data', type=float, default=1.0, help='Weight for data loss')
    parser.add_argument('--w-res1', type=float, default=1.0, help='Weight for physics residual 1')
    parser.add_argument('--w-res2', type=float, default=1.0, help='Weight for physics residual 2')
    parser.add_argument('--w-res3', type=float, default=1.0, help='Weight for physics residual 3')
    parser.add_argument('--w-res4', type=float, default=1.0, help='Weight for physics residual 4')
    parser.add_argument('--w-mass1', type=float, default=1.0, help='Weight for mass constraint 1 (if enabled)')
    parser.add_argument('--w-mass2', type=float, default=1.0, help='Weight for mass constraint 2 (if enabled)')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Constant Weight PINN Training')
    ...
    # Collect method-specific parameters (weights)
    method_params = {
        'w_data': float(args.w_data),
        'w_res1': float(args.w_res1),
        'w_res2': float(args.w_res2),
        'w_res3': float(args.w_res3),
        'w_res4': float(args.w_res4),
    }
    if not args.synthetic:
        method_params.update({
            'w_mass1': float(args.w_mass1),
            'w_mass2': float(args.w_mass2),
        })
    
    # Setup device and output directory
    device = setup_device(config['device'])
    output_dir, model_path = create_output_directory('constant_weight_pinn', config['output_dir'])
    
    # Print configuration
    print_training_config(config, 'Constant Weight PINN', method_params)
    ...
    # Initialize loss with parsed weights
    loss_weights = {
        'data': method_params['w_data'],
        'phys_res1': method_params['w_res1'],
        'phys_res2': method_params['w_res2'],
        'phys_res3': method_params['w_res3'],
        'phys_res4': method_params['w_res4'],
    }
    if not args.synthetic:
        loss_weights.update({
            'phys_mass1': method_params.get('w_mass1', 1.0),
            'phys_mass2': method_params.get('w_mass2', 1.0),
        })
    loss_method = ConstantWeightLoss(enable_mass_constraints=not args.synthetic, weights=loss_weights)
    ...
    for epoch in tqdm(range(config['epochs']), desc="Training"):
        ...
        with torch.no_grad():
            for xb, yb in data['val']:
                ...
                # Use the same loss structure as in training for consistency
                data_loss_val = loss_components[0]
                phys_losses_val = torch.stack(loss_components[1:])
                total_val = data_loss_val + torch.mean(phys_losses_val)  # Make consistent with training
    ...
    np.savez(os.path.join(output_dir, 'constant_weight_history.npz'), **hist)
    plot_training_history(hist, output_dir, 'Constant Weight PINN', 
    ...
    plot_trajectory_predictions(model, data, output_dir, 'Constant Weight PINN', device=device)
    ...
    np.savez(os.path.join(output_dir, 'constant_weight_parameters.npz'), **final_params)
    
    # Save ground truth parameters for comparison (if available)
    if 'metadata' in data and data['metadata'] is not None:
        if 'pinn_parameters' in data['metadata']:
            ground_truth = data['metadata']['pinn_parameters']
            np.savez(os.path.join(output_dir, 'ground_truth_parameters.npz'), **ground_truth)
            print(f'Ground truth PINN-compatible parameters saved for comparison')
        elif 'parameters' in data['metadata']:
            ground_truth = data['metadata']['parameters']
            np.savez(os.path.join(output_dir, 'ground_truth_parameters.npz'), **ground_truth)
            print(f'Ground truth original parameters saved for comparison')
